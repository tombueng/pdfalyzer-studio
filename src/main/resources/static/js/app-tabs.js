...
PDFalyzer.Tabs = (function ($, P) {
    ...
    function renderFontDiagnostics() {
        ...
        html += '</tbody></table>' +
            '<div id="fontDiagDetail" class="font-diag-detail text-muted">Click a font row to open the glyph mapping table.</div>' +
            '</div>';
        $c.html(html);
    ...
    function buildMappingRowHtml(row, state) {
        ...
        var glyphPreview = '<span class="font-glyph-lazy" ' +
            ...
            '</span>';
        return '<tr class="font-map-row" data-code="' + escapeHtmlAttr(String(row.code == null ? '' : row.code)) + '" data-unicode="' + escapeHtmlAttr(row.unicode || '') + '">' +
    ...
    function setupDeferredFontDetailRendering() {
        ...
        var mapSection = document.getElementById('fontDetailMapTable');

        function renderVisibleSections() {
            if (mapSection && isElementNearViewport(mapSection)) {
    ...
    function initGlyphCanvasViewer(canvas, viewport, metricsEl) {
        ...
        function drawCurrent() {
            ...
            drawGlyphDiagnosticsCanvas(canvas, {
                ...
                logicalWidth: Math.round(activeGlyphCanvasState.baseWidth),
                logicalHeight: Math.round(activeGlyphCanvasState.baseHeight)
            });
            canvas.style.width = Math.round(activeGlyphCanvasState.baseWidth * activeGlyphCanvasState.zoom) + 'px';
            canvas.style.height = Math.round(activeGlyphCanvasState.baseHeight * activeGlyphCanvasState.zoom) + 'px';
            updateZoomLabel();
        }
